...
new Elysia().use(swagger()).use(users).use(feed).listen(3000)
```
...
const app = new Elysia()
  .patch(
    '/user/profile',
    ({ body, status }) => {
      if (body.age < 18) return status(400, 'Oh no')

      if (body.name === 'Nagisa') return status(418)
...
new Elysia().get('/', 'Hello Elysia').get('/video', file('kyuukurarin.mp4')).listen(3000)
```
...
new Elysia().get('/', ({ status }) => status(418, 'Kirifuji Nagisa')).listen(3000)
```
...
::: tip
We **highly recommend** you to read this page before learn more about Elysia.
:::
...
const app = new Elysia().get('/', () => 'Hello from Elysia').mount('/hono', hono.fetch)
```
...
new Elysia().use(swagger()).post('/sign-in', ({ body }) => body, {
  body: t.Object(
    {
      username: t.String(),
      password: t.String({
...
describe('Modules', () => {
  it('inline async', async () => {
    const app = new Elysia().use(async (app) => app.get('/async', () => 'async'))
...
new Elysia().get('/ai', ({ query: { prompt } }) =>
  fetch(endpoint, {
    method: 'POST',
    headers: {
...
Alternatively, you can download llms.txt or llms-full.txt and feeds it to your favorite LLMs like ChatGPT, Claude or Gemini to get a more interactive experience.
...
export const note = new Elysia({ prefix: '/note' })
  ...
  .put(
    '/',
    ({ note, body: { data }, username }) => note.add({ data, author: username }),
    {
      body: 'memo'
    }
  )
